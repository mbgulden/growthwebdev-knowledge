---
type: Reference
title: "Agy Implementation Plan"
description: "Mirrored from Google Drive on 2026-06-25. Source: Drive file 1xs-HwJw2FUwc8onEuBmNS78JQRFBmfkuDU66A4RM3wE (modified 2026-06-08). Originally part of the Prismatic source plugin plans and AGY architecture reports."
resource: https://docs.google.com/document/d/1xs-HwJw2FUwc8onEuBmNS78JQRFBmfkuDU66A4RM3wE/edit?usp=drivesdk
tags: [drive-mirror, prismatic, gemini-evaluation, agy-report, source-plugin-plans]
timestamp: 2026-06-25T04:04:35.849Z
git_repo: mbgulden/growthwebdev-knowledge
linear_issue: TBD
last_verified: 2026-06-25
verified_by: fred
status: current
drive_file_id: "1xs-HwJw2FUwc8onEuBmNS78JQRFBmfkuDU66A4RM3wE"
drive_modified: "2026-06-08T10:10:34.310Z"
---
...
\>   
\> \*\*The Resolution:\*\* All lock keys in \`swarm\_locks.json\` must be stored as \*\*repository-relative paths\*\* (e.g., \`src/components/Nav.tsx\`) resolved against the git root directory.

\#\#\#\# 2\. Local vs. Distributed Lock File  
\> \[\!IMPORTANT\]  
\> If agents work in separate repository directories on the same server, reading/writing a local \`.antigravity/swarm\_locks.json\` file inside their own checkout folder isolates their lock registries. They will not see each other's locks.  
\>   
\> \*\*The Resolution:\*\* Centralize the lock database file. The engine must support a global locks directory via an environment variable \`SWARM\_LOCKS\_DIR\` (defaulting to \`/home/ubuntu/.antigravity/swarm\_locks.json\` or \`/tmp/swarm\_locks.json\`).
...
sequenceDiagram  
    ...
    participant DB as Global swarm\_locks.json  
...
\#\#\# Phase 1: Declarative Rules & Lanes (Immediate)  
\- Author \`PRISMATIC\_ENGINE.yaml\` inside target repositories.  
...
\- Refactor \`.antigravity/swarm.js\` to:  
  \- Read/Write to a centralized database (defaulting to \`/home/ubuntu/.antigravity/swarm\_locks.json\`).  
...
\- The hook will:  
  1\. Detect the pushing agent via environment variable (\`HERMES\_AGENT\_ID\`).  
  ...
  4\. Match modified files against \`PRISMATIC\_ENGINE.yaml\` lanes.  
  5\. Check central \`swarm\_locks.json\` to verify the active agent holds locks for those files.
...
1\. \*\*Deploy configuration file:\*\* Place \`PRISMATIC\_ENGINE.yaml\` at repo root:  
   \`\`\`yaml  
   version: 1  
   agents:  
     fred:  
   ...
   staging:  
     ...
     governor: "fred"  
   \`\`\`  
2\. \*\*Deploy central CLI lock manager:\*\* Copy and modify \`swarm.js\` to \`/home/ubuntu/.antigravity/swarm.js\` pointing to \`/home/ubuntu/.antigravity/swarm\_locks.json\`.  
