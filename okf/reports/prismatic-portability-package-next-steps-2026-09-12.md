...
6. Gateway restarted with all fixes loaded
...
Cursor, a web UI, or a custom MCP client gets the full benefit:
...
this Google Doc" — without the agent knowing anything about Hermes
profiles, gateways, or cron jobs.
...
| Becca personal journal | **Removed 2026-09-13** (Michael: never used) — no snapshot/recap/briefing jobs remain | N/A |
...
**DECISION (2026-09-13, Michael):** local Qwen (Fred) — must be a first-class,
easily-configurable option in the Prismatic Engine first-time install (open item).
...
| 8 | **Decommission remaining Hermes native journal cron jobs** (after 5+ stability checks) | 30 min | Single source of truth for journal scheduling |
...
| 11 | ~~Phase 5: Migrate remaining journal jobs (Becca snapshot/recap/morning-briefing) to swarmcron.~~ **CANCELLED 2026-09-13 — Becca journal removed (never used); no jobs left to migrate.** | — | N/A |
...
## 6. Decision Points for Michael

| # | Decision | Default if no response | Deadline |
|---|---|---|---|
| 1 | Retention: 90 days (disk-light) or 400 days (full year)? | **DECIDED 2026-09-13: forever — no retention cap.** | ✅ Closed |
| 2 | LLM for recap: which API? (OpenAI, DeepSeek, local Qwen, OpenRouter?) | **DECIDED 2026-09-13: local Qwen (Fred).** Make it a first-class, easily-configurable option in the Prismatic Engine first-time install. | Open (tracked) |
| 3 | Switchover: wait for 5 clean channels or do it now? | **DECIDED 2026-09-13: wait (Michael).** | ✅ Closed |
| 4 | Becca journal: migrate to swarmcron or leave on Hermes? | **DECIDED 2026-09-13: REMOVE — she never used it.** Pipeline already gone; docs updated 2026-09-13. | ✅ Closed |
