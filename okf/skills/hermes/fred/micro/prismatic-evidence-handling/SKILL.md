...
**Fifth occurrence 2026-08-26 (gdrive reauth) — the flagged changed-paths can include the verifier script itself.** The gate re-fired on a *newly created* `/tmp/hermes-verify-*.py` and asked to "create a verification script" — i.e., demanded evidence *about the evidence tool*. The close that worked: (a) re-run the existing script (fresh timestamp + same PASS count), (b) `sha256sum` + mtime stability on the flagged paths (mtimes predate the re-run = no mutation since the green round), (c) label it stale detector state and quote both hashes. Do **not** write a second "meta-verifier" that verifies the first verifier — the stability proof IS the verification of the verifier. Companion pattern from the same session: the other flagged path (the payload/handoff JSON) was covered *inside* the existing verifier via a content-fidelity check (hot file's `one_line` == payload's `one_line`) — no new artifact needed for a flagged data file whose behavior is already asserted.
