...
**Local-model rollout recipe:** replace the hardcoded `model: provider: minimax / default: MiniMax-M3` block with the local llama.cpp provider (`api: http://192.168.1.230:8000/v1`, model `local-qwen-27b-q8-fred`, `request_timeout_seconds: 600`) across all 4 sources + live dirs; patch `build_usage` env defaults in guest_agent_server.py; back everything up (`*.bak-prelocalmodel`). Guest dirs are chowned uid 1000 → edits need `sudo -n`.

**vLLM key (GRO-4929, 2026-09-07 — SUPERSEDES `api_key: llama-local`):** the `:8000` vLLM 0.27.1 server is a **single-key** server configured with `VLLM_FRED_API_KEY`; the old `api_key: llama-local` literal does **not** authenticate (guests 401). The config must be env-driven, never a literal: set `api_key_env: GUEST_VLLM_API_KEY` under the provider block, and have the orchestrator's per-user `.env` template inject `GUEST_VLLM_API_KEY={os.getenv("GUEST_VLLM_API_KEY") or os.getenv("VLLM_FRED_API_KEY","")}`; the guest `docker-compose.guest.yml` must pass `GUEST_VLLM_API_KEY=*** into the container env (otherwise compose interpolates empty → guest 401). All 3 touchpoints: the `config_content` string in `vm_orchestrator.py` (the authoritative, provision-time source), the template `docker-compose.guest.yml`, and the template `config.yaml`.

**Tenant capacity guardrail (2026-09-07):** keep guest `context_length` at **65536**, not 262144. The shared Fred pool on 192.168.1.230 holds ~383K KV tokens (fp8): 65536/tenant → ~6 concurrent tenants; 262144 → only ~1.4 full-length conversations, at which point every other tenant/agent preempts. This was a deliberate Michael decision — do not "fix" 65536 back up. (Note the merged `main` generator still carries 262144 pending the alignment PR; prod runs 65536.)

**Prod cutover is NOT "sync main + restart."** Prod's daemon (`hde_orchestrator.service`, uvicorn on 127.0.0.1:8001) runs an **UNTRACKED local** `scripts/vm_orchestrator.py` in the `hd-platform/` checkout (on a `ned/...` branch, with uncommitted doc .html files) and provisions guests from the **`/home/ubuntu/guest_hermes_bot/`** fallback template — NOT `scripts/guest_hermes_template/` (absent in the prod checkout) and NOT whatever `main` has. A branch switch / `git checkout main -- .` would (a) risk the uncommitted docs and (b) ship main's 262144 over prod's 65536. Correct cutover: back up the 3 files, **surgically patch** the live untracked `vm_orchestrator.py` (config generator `api_key_env` + `.env` injection line) and the `/home/ubuntu/guest_hermes_bot/` template (compose passthrough + config `api_key_env`), keep the 65536 guardrail, then `sudo systemctl restart hde_orchestrator.service`. The daemon reads `ORCHESTRATOR_PORT` (default 8001; unset in prod) — journalctl "Uvicorn running on http://127.0.0.1:8001" is the boot-readiness signal (`/` is 404 by design; there is no root route).

**"Verify the hits" E2E after a prod restart (the real readiness proof, not just /ping):** provision a throwaway guest via the HMAC endpoint and inspect what actually got generated, then deprovision:
1. `POST http://127.0.0.1:8001/api/orchestrate/provision` with `X-Signature: hmac-sha256(ORCHESTRATOR_SHARED_SECRET, raw_body)` and JSON `{user_id:999, action:"provision", telegram_user_id, guest_bot_token, guide_name, access_status}` → expect 200 `{success:true}`.
2. Assert `/home/ubuntu/guest_hermes_bot_999/.env` has **non-empty** `GUEST_VLLM_API_KEY` that **== the server's `VLLM_FRED_API_KEY`** (the orchestrator fell back correctly).
3. Assert the generated `config.yaml` has `api_key_env: GUEST_VLLM_API_KEY` (not the literal) and the correct `context_length`; assert the generated `docker-compose.yml` has the `GUEST_VLLM_API_KEY` passthrough.
4. **Real engine hit:** `POST http://192.168.1.230:8000/v1/chat/completions` with `Authorization: Bearer <guest key>` → **200**; no-key control → **401**. (The 200-with/401-without pair is the auth proof, not the response body.)
5. `action:"deprovision"` → 200; confirm container + `/home/ubuntu/guest_hermes_bot_999/` + `/home/ubuntu/users/guest_999/` are gone and the N other guests are untouched.
Note: this vLLM returns `reasoning_content`, not `content`, for the 27B model — assert on the HTTP status, not the parsed body.
