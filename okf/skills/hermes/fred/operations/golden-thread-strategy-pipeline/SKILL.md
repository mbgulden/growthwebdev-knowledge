...
7. **Execute the top task through AGY or directly when the task is infrastructure/demo-gate work**
   ...
   - For stale blocker closeout, live-check Linear first, implement only the missing acceptance slice, publish via a clean PR if needed, post exact evidence, move only evidenced issues to Done, and remove stale routing labels (`dispatch:ready`, `agent:peer-review`, `agent:needs-human-review`) that would keep completed work surfacing as blocked. See `references/blocker-closeout-linear-pr-2026-07-09.md`. **If the selected ticket's open state is STALE because the fix already landed** (Phase 0.5 live-verify shows the commit is on `origin/main`, live prod serves the correct state, and the guard PASSES — but the ticket still sits In Progress/Todo), that is a **closeout, not a build**: post an evidence comment, move the ticket to Done via `issueUpdate(id, input:{stateId})` (note: `id` is a top-level arg, `stateId` UUID inside `input`), and read the state back. Do NOT create a new tree or re-implement. See `references/stale-ticket-live-closeout-2026-09-13.md` (includes the Linear GraphQL gotchas hit: `orderBy: priority` invalid, no `IssueFilter.identifier`, `issueUpdate` id top-level, multi-line query JSON control-char error).
...
- **Do not trust a stale Active Oahu DNS/404 premise from the registry or AGY research.** Fresh-check apex and mirror URLs before creating cutover tasks; if cutover is complete, correct the registry and move the strategy toward CRO and conversion measurement. See `references/active-oahu-dns-cro-reconciliation-2026-07-11.md`.
- **A Golden Thread cron re-verify is independent of the handoff file — a concurrent-writer race does NOT invalidate your live check (observed 2026-09-13, run 12).** The recurring golden-thread cron reads `state/current.json`, live-re-verifies the gated issue (GRO-4929 this run), then writes the handoff back. If a *parallel* cron instance of the same job is running, it can rewrite `current.json` between your two reads (run numbering drifts, the file shrinks/reshapes). Two rules: (1) **Your live Linear re-query is a claim about Linear's state, not about the file** — it stands regardless of the race; report the verify normally (e.g. "12th consecutive zero-drift hit"). (2) **Re-read `current.json` immediately before you write it** (or use `handoff.py write` / `write_and_wire.py`, which temp-file-replace atomically and archive the prior version) so you don't clobber a parallel writer's just-landed state. Flag the race in `notes_for_next_self` for a single-flight-guard audit — do NOT serialize the cron or change the job definition from inside a cron run. Full recipe + the observed field-layout changes: `agent-operations/session-state-handoff` Pitfalls → "concurrent-writer race on `state/current.json`".
...
- `references/stale-ticket-live-closeout-2026-09-13.md` — pattern for when Phase 0.5 catches a Linear ticket whose open state is STALE because the fix already landed (commit on `origin/main`, live prod serves the correct state, guard PASSES, but the ticket still sits In Progress/Todo): verify read-only, post an evidence comment, move to Done via `issueUpdate(id, input:{stateId})` (id top-level, stateId UUID in input), read back. Also captures the Linear GraphQL gotchas hit building it (`orderBy: priority` invalid, no `IssueFilter.identifier`, `issueUpdate` id top-level, multi-line query JSON control-char error).
