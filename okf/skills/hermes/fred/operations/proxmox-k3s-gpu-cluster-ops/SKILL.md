...
10. **vLLM on dual RTX 3090 — `lued/Qwen3.8-27B-INT8-W8A16-MTP` is the verified-good starting point.** The lued INT8 W8A16 + MTP variant (~31.6 GB, 1999 safetensors files with MTP head preserved) is specifically validated for vLLM on dual RTX 3090 with PCIe (no NVLink), with measured 14.85 GiB per GPU, 266k shared KV pool, MTP speculative decoding (3 draft tokens, ~85% acceptance rate). vLLM's wins over llama.cpp are concurrency + speculative decoding + PagedAttention KV memory, not single-stream speedup (which is modest ~2-3×). **Status update 2026-08-24/26: the single-GPU AWQ gap that kept Kai/Ned on llama.cpp was filled by a public checkpoint (`barrydeen/Qwen3.8-27B-AWQ-4bit`, full VL) — Ned has been on vLLM (:8003, GPU 2+3) since 2026-08-24; only Kai (.232:8080) remains on llama.cpp.** For "how many profiles can share an engine" see `references/vllm-multi-profile-capacity-planning.md`. For the full vLLM recipe, command-line, and trade-off analysis see `references/vllm-via-lued-int8.md`.
...
13. **"How many Hermes profiles can run on the local model?" — size the ENGINE, not the profiles.** Profiles are unbounded (config dir + ~0.5–1 GiB gateway each); the constraint is the single inference engine's `max-num-seqs` + shared KV pool. Answer only after re-grepping every profile config for its `api:` base URL + `default:` model (the consumer map drifts), probing `/v1/models` for served IDs + `max_model_len`, and running live `nvidia-smi` on every GPU box (.230 AND .232) for allocation + headroom. Comfortable active concurrency ≈ the measured batch-4 point (4–6 profiles); 10–12 works with queuing; idle profiles cost zero. Expansion ladder: same endpoint → KV/seqs config → second engine (`.232`'s single 3090 fits the AWQ-4bit 27B at ~128K ctx). See `references/vllm-multi-profile-capacity-planning.md`.
...
- `references/vllm-multi-profile-capacity-planning.md` — how to answer "how many Hermes profiles can work on the local model?": profile count is unbounded; size the engine (`max-num-seqs` + shared KV + live nvidia-smi across .230/.232) instead. Includes the consumer-map audit recipe, the 4–6 active / 10–12 workable heuristics, the expansion ladder, and the 2026-08-26 endpoint snapshot. Load when Michael plans to put "everyone to work" or asks what his limits are for a shared local engine.
