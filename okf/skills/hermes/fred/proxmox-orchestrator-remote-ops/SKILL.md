...
- **On PVE 9.x, per-VM status via the ticket-auth API returns `Method 'GET /qemu/<id>/status' not implemented` even with a valid ticket** (2026-08-21, VM 232 on pve3: `/qemu/232/status/current` and `/qemu/232/status` both returned "not implemented" while the 4-step probe's `/nodes` read worked fine). This is a distinct failure mode from the 401-on-POST trap — the GET succeeds auth-wise but the endpoint is missing. Don't burn time debugging headers or trying endpoint variants for VM status. **Switch to SSH + `qm` immediately**: `sshpass -p <pw> ssh root@<node> 'qm list | awk "$1==<vmid>"'` (node identity) and `qm status <vmid>` / `qm config <vmid>` (state). Cluster-wide discovery (`/nodes`, `/cluster/resources`) still works via the API; per-VM reads go over SSH.
