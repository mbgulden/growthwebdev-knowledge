...
## Flapping local-LLM watchdog alerts

When Michael reports his/Kai's setup "going on and off" with a watchdog alert like `keyed completion probe failed (no output)`, do not assume the server is down. Follow `references/llm-watchdog-false-positive-flap.md`: classify the flap from the cron job's per-run output files, prove no real outage via `ActiveEnterTimestamp`/`NRestarts`/OOM/port checks, prove load via slot `print_timing` journal density, re-run the exact probe timed, and explain correlated multi-profile alerts as shared-backend contention. The autobot watchdog was patched 2026-08-22 to the recommended shape (split `PROBE_TIMEOUT=180`, `MIN_STRIKES=2`, distinguished failure text, `{down, strikes, last_alert}` state) — the reference carries the applied baseline plus a unit-test recipe (temp state file, `wd.time.time` clock fake, `check_target` monkeypatch, exact stdout+dict assertions) for verifying any future edit to a `no_agent` cron watchdog without waiting for live ticks.
...
- `references/llm-watchdog-false-positive-flap.md` — session-derived recipe for diagnosing flapping local-LLM watchdog alerts ("probe failed, no output") as load-timeout false positives rather than server outages.
