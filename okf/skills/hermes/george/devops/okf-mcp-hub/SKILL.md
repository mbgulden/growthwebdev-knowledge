...
- **Idle-window reload rule:** after a manual merge to main, do NOT fire `/reload-mcp` into active agent lanes — Michael pushes reloads himself when agents are idle. Report staleness instead of triggering.
...
- After a merge lands, running profile servers are stale until reload/new session — say so in the report. Local `main` also diverges (pre-squash lineage; `pull --ff-only` fails) — sync per `branch-deletion-approval` → `references/squash-merge-verification.md`.
