...
Use this reference when a Prismatic slice is a provider/gateway/status adapter around an already-existing core verification or policy engine.

## Session-derived signal

A preflight may reveal that the first task draft mixes separable concerns, for example:
