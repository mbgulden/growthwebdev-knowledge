...
9. **Memory health (2026-08-20):** `memories/MEMORY.md` + `USER.md` have hard char caps (~2200 / ~1375). **Over-cap = the profile's memory writes are silently rejected** — the symptom is "agent is forgetting things," not an error log. Check with `wc -c` on both files; the daily swarm-journal digest also reports over-cap files. Fix: prune to pointers (see `okf/standards/hermes-memory-skills-boundary-discipline.md`). Also detect orphaned legacy-format `memory.jsonl`/`user.jsonl` in the same dir — pre-boundary-discipline format, not loaded by current memory injection; report as orphaned, don't treat as live memory.
