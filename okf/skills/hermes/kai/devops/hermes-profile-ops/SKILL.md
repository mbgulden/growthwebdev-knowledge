...
description: "Operate multiple Hermes profiles on a shared box: mirror one profile's model stack onto others (config parity), restart gateways safely via systemd, diagnose a silent/unresponsive profile (dead gateway, Restart policy, Telegram polling conflicts, or responsive-but-wrong-provider silent fallback), and run fleet-wide disk/cleanup sweeps. Use when: aligning a new/old profile to a known-good config, 'make X's setup like Y's', restarting a hermes gateway, 'why is X not responding on Telegram', or Michael asks 'what cleanup / optimal surface'."
category: devops
...
7. Probe the profile's inference endpoint directly before restarting, so a silent model isn't masked by the gateway fix: `curl -s -m 60 <base_url>/v1/chat/completions` with a 50-token "reply FRED_OK" prompt.
8. **Responsive but wrong model** — the profile answers fine on Telegram, but `grep -a 'LLM call completed' <profile>/logs/agent.log | tail` shows `provider=<fallback>` (e.g. `gemini`) with repeated `BadRequestError` against the local endpoint: the primary endpoint is rejecting every request and the fallback chain is silently absorbing it. Cost tell: a profile supposed to run on a local endpoint burning 1M+ tokens/turn is running on the fallback. Prime suspect: `role: system` entries in the prefill file (vLLM 400s on dual-system payloads) — see Pitfalls + `references/vllm-prefill-system-message-400.md`.
...
- **vLLM backend + `prefill_messages_file` containing `role: system` = every turn silently falls back.** vLLM rejects any request with more than one system message (`400 "System message must be at the beginning."`), and Hermes injects prefill *after* the real system prompt (conversation_loop.py ~line 842) — so a prefill file with even one system entry produces a dual-system payload on every call, and the fallback chain (e.g. Gemini) serves every turn with no visible outage. Fix: rewrite prefill entries to `role: user` (content verbatim), re-probe the endpoint with the corrected shape, then restart the gateway — prefill is loaded once at GatewayRunner init (gateway/run.py ~line 2548) and cached in-process, so a file edit alone changes nothing (MainPID-kill pattern from Section B). Generic staleness check: config mtime newer than gateway process start ⇒ the running process is on the old config. Full trace: `references/vllm-prefill-system-message-400.md`.
