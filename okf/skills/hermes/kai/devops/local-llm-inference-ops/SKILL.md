...
## vLLM 401 / stale consumer key diagnostics (observed 2026-09-06)

Watchdog cron 401'd on fred `:8000` + ned `:8003` the morning after a `.230` key rotation. Diagnose with temporal correlation FIRST, key comparison second:

1. **Service restarts near failure?** `ssh root@192.168.1.230 'systemctl show -p ActiveEnterTimestamp <unit>'`. No restart near the timestamp → not an engine startup issue.
2. **Key-file mtimes on the service host**: `stat -c '%y' /opt/vllm_bin/.api_keys_*`. A rotation (new mtime) before the failure timestamp = consumer key probably stale.
3. **Consumer `.env` mtime** vs failure: `.env` edited the same night as the rotation = the exposure window. **Any `.230` key rotation not mirrored into every consumer's `.env` 401s until mirrored** — services themselves are healthy.
4. **Live auth matrix (compare lengths/prefixes only — never store values):**
   - unauthenticated `curl /v1/models` → expect 401 (keying active);
   - live service key from `/proc/<pid>/cmdline` (length+prefix only; pipe it straight into `curl -H "Authorization: Bearer …"` in one step — never extract→`rm`→`scp`, that pipeline races) → own port 200, cross port 401;
   - consumer `.env` key → each port: 200 = fixed, 401 = stale.
5. **Verify the fix in the exact cron runtime** (see `scheduled-reminder-management` → diagnosing cron failures): `env -u VLLM_FRED_API_KEY -u VLLM_NED_API_KEY HOME=<gateway HOME> python3 <script>` → expect exit 0.

Session detail + full timeline: `references/vllm-401-watchdog-2026-09-06.md`.
...
- **`no_agent` watchdog scripts: resolve `.env` absolutely, never via `~`/`Path.home()`** (observed 2026-09-06): the agent shell has a redirected `HOME` (profile home) while the gateway spawning `no_agent` cron has `HOME=/home/ubuntu` — the same `~`-based path resolves to DIFFERENT (or nonexistent) files in each context. A watchdog that only works under the interactive shell will 401 in cron. Fix: try the script-adjacent absolute path first (`os.path.dirname(__file__)`-anchored), fall back to `~`, and record which source the key came from (`env` vs `env-file:<path>` vs `none`) so a future 401 is diagnosable from the alert text alone. Verify with `env -i … HOME=<gateway HOME> python3 <script>` AND a hostile `HOME=/nonexistent` run.
- **Diagnostic-label trap — read YOUR dict before suspecting the environment (observed 2026-09-06):** a freshly-added `key_source` field printed `None` on a HEALTHY endpoint (`alive=True`), which looked like an impossible env-inheritance mystery and triggered `/proc/<pid>/environ` archaeology on the gateway PID. Actual cause: my new lookup dict was keyed `"fred"`/`"ned"` while the caller passed `"fred-vllm"`/`"ned-vllm"` — a `.get()` miss returns `None` silently while the live probe still passes. Rule: when a diagnostic you just wrote contradicts a live probe (None-but-alive, 401-but-200, missing-but-found), the FIRST suspect is the label/lookup code you added, not the frozen gateway env. Re-read the dict + the call sites before opening `/proc`.
- **Patch-mangle discipline for Python scripts (observed 2026-09-06):** two consecutive fuzzy `patch` calls on `llm-engine-watchdog.py` corrupted it in sequence — first dropped the debug block (silent no-op), second mangled the `__main__` tail. After ANY fuzzy patch to a Python file: `python3 -m py_compile` + `grep` for the expected symbols + re-read the file tail. If structure is mangled, stop patching — a full `write_file` rewrite from the verified good content is faster and safer than a third fuzzy attempt.
...
- `references/vllm-401-watchdog-2026-09-06.md` — full 401 timeline, evidence, and the APPLIED hardening of `llm-engine-watchdog.py` (HOME-proof `.env`, key-source labels, 401 detail, single alert) + the two self-inflicted bugs found during it (lookup-key mismatch, fuzzy-patch mangling).
