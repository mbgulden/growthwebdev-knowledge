...
- **vLLM with `--reasoning-parser qwen3` puts the answer in `reasoning`, not `reasoning_content`.** Field names differ by backend: llama.cpp → `reasoning_content`; vLLM 0.27.x → top-level `reasoning` on the message, with `content: null` while the model is still "thinking" within budget (observed 2026-08-24 on the Ned cutover, vision round-trip: `content: null` + full answer in `reasoning`). Health probes and harnesses must read BOTH fields or they report healthy endpoints as empty.
