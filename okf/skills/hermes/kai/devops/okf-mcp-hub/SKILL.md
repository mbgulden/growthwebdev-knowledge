...
- The MCP never writes. Write path = git on the checkout:
  ...
  2. Branch (George's prefix is `george/`; Fred stages staging). **Direct main push is blocked** — branch → PR → manual merge (Michael merges). Before branching, run `mcp_okf_status`: if `dirty_files > 0`, `git status` in the checkout and identify the dirty file FIRST — the shared worktree is not yours alone, and branching on top of someone else's uncommitted changes bakes their state into your PR base.
...
- After a merge lands, running profile servers are stale until their process respawns. Do NOT restart `okf-mcp.service` expecting your session to pick it up — see the "Per-profile staleness" pitfall for the `kill <stdio-child-pid>` refresh recipe (verified 2026-08-23).
...
9. **Post-merge:** running MCP servers are stale; `mcp_okf_search` on the new domain is the acceptance check — but only AFTER a respawn (see "Per-profile staleness" pitfall for the kill-recipe; restarting the standalone HTTP service does not refresh your session's stdio child).
...
10. **The hub push is lane-gated, and the resolution path matters (hit live 2026-08-20).** The commit lands locally fine; the pre-push hook blocks the *push* (it validates files between the local and remote SHAs). Phase 3's commit had 51/60 files outside kai's lane (content docs under `okf/projects/<name>/<subpath>/` + `okf/decisions/`). The designed resolution, in order: (a) **authorization decision + lane edit** (most durable — future pushes stay hook-clean): Michael's explicit authorization → new/edited lane entry in `PRISMATIC_ENGINE.yaml` (the hook reads repo root at push time; `decisions/okf-agent-commit-authorization.md` precedent); (b) **hand the branch to a wildcard-lane agent** (fred/george own `*`); (c) **explicit user authorization in chat → bypass + disclose in the PR body** (PR #30 root-index, PR #32 Phase 3 spoke docs, and PR #38 journal-mcp integration doc 2026-08-21 used this — acceptable one-shot escape when Michael says the word, but the authorization lives only in chat/PR body and the lane gap stays for future pushes; prefer (a) for recurring out-of-lane work). Exact bypass that works: `git -c core.hooksPath=/dev/null push -u origin <branch>` (`-c` MUST precede `push` — trailing flags get parsed as push options and the command dies on usage). Disclosure standard (hit live 2026-08-21): put a "LANE WAIVER" section in BOTH the commit message and the PR body naming the out-of-lane path, the missing lane owner (e.g. Jules offline), and the user's approval quote verbatim. (d) **hold** — content stays locally committed and is still live-searchable because MCP servers read from the working tree, but the branch doesn't exist on GitHub. Never `--no-verify` without explicit user authorization, and NEVER unilaterally widen your own lane in the yaml — that defeats the control.
...
15. **The shared-checkout race has a second, nastier variant: BRANCH-POINTER switches mid-operation (hit live 2026-08-20, twice in one session).** A concurrent agent can `git switch` the ONE shared checkout while you're mid-sequence, so (a) YOUR `git commit` lands on THEIR branch (my drift commit landed on `feature/fred-okf-hde-guest-fleet-ops`), and (b) THEIR commit lands on YOUR branch (Fred/Ned's `909b505` landed inside my open PR #35). Consequences: your PR ships foreign work (lane violation + scope leak), and the foreign agent's push ships your work. Recovery recipe: (1) `git branch -f <their-branch> <their-clean-sha>` to restore their pointer BEFORE they push; (2) create a local safety ref (`git branch -f rescue/<desc> <foreign-sha>`) so the foreign commit can never dangle; (3) `git cherry-pick <foreign-sha>` onto their restored branch, verify the resulting file is byte-identical (`git rev-parse <a>:<path>` vs `git rev-parse <b>:<path>`), push their branch; (4) REBUILD your branch: `git reset --hard origin/main` then `git cherry-pick <your-sha-1> <your-sha-2> ...` (cherry-pick has NO `-q` flag — `-q` is a usage error); (5) `git push --force-with-lease` on your own branch only; (6) prove the PR is clean: `git diff --name-only origin/main..HEAD | grep -vE '<your-lanes>'` → empty. Pre-empt the whole class: before ANY commit on a shared checkout, assert `git branch --show-current` is still yours in the SAME command as the commit (`BR=$(git branch --show-current); [ "$BR" = mine ] || exit 1`), and re-assert after (the switch can happen between commands). Note: a cherry-pick onto a branch whose base already contains the file applies only the delta (`git show --stat` will show fewer files than the original commit) — verify by path, not by file count.
...
## Standalone service `okf-mcp.service` (verified 2026-08-20 — portability)

The OKF server has TWO transports, and only one of them is orchestrator-independent:

| Transport | How it runs | Survives orchestrator/gateway death? |
|---|---|---|
| `okf-mcp.service` (systemd) | `/etc/systemd/system/okf-mcp.service`, `User=root`, `Restart=on-failure`, env from `/etc/okf-mcp/mcp.env` (mode 600, root-only) | **YES** — parented by PID 1. Proof: orchestrator's gateway unit was in `failed` state while the service kept serving. |
| per-gateway stdio child | Each profile gateway spawns its own `server.py` child | NO — dies with its gateway. These are the fragile ones. |

Standalone service facts: `MCP_TRANSPORT=streamable-http`, `HOST=127.0.0.1`, `PORT=8910`, `OKF_ROOT=<hub checkout>`, `OKF_ALLOW_UPDATE=0` (default), bearer-gated (`MCP_BEARER_TOKEN` in the root-only env file), open `/healthz` (no auth — liveness for monitoring), `MCP_ALLOWED_HOSTS`/`MCP_ALLOWED_ORIGINS` set for the `okf.growthwebdev.com` tunnel.

- **Index freshness:** in-memory, built at process start. After ANY hub commit (merge, cron regen), the service serves a stale index until `sudo systemctl restart okf-mcp.service`. The daily regen cron now does this automatically after a successful push (added to `skill-hub-regen.sh`, 2026-08-20).
- **Kai has passwordless sudo** for this service (verified 2026-08-20: `sudo -n systemctl restart okf-mcp.service` works non-interactively).
- **Wiring a profile client to the standalone HTTP endpoint instead of a stdio child:** config schema is `mcp_servers.<name>: {url: "http://127.0.0.1:8910/mcp", headers: {Authorization: "Bearer ${MCP_OKF_API_KEY}"}, enabled: true}` (Hermes expands `${ENV_VAR}` in header values); the token value goes in the profile's `~/.hermes/profiles/<p>/.env` as `MCP_OKF_API_KEY` (the CLI's env-key convention for server `okf` is `MCP_OKF_API_KEY`). `hermes mcp add --url ... --auth header` does this interactively. `/reload-mcp` (in-chat) or a new session picks up the change — no gateway restart.
- **Bearer-gate gotcha (hit live 2026-08-20):** the gate compares `hmac.compare_digest(got, "Bearer " + expected)` where `expected` is `os.environ["MCP_BEARER_TOKEN"].strip()` **captured at process start**. If the env file is edited after the service started, the running server still checks the OLD token (and vice versa). A 401 with "the correct" token almost always means env-file ≠ running-process token — compare `sudo cat /proc/<pid>/environ` (live) vs `sudo cat /etc/okf-mcp/mcp.env` (file) by hash only, never echo either. **Shell-quoting kills token handling**: the token contains special chars — `cut -d= -f2`, `sed 's/^...=//'`, and `tr` pipelines silently produce empty strings (the `=` and following chars get eaten by the sed/cut interpretation). Always extract tokens via Python (`sudo cat` + `line.split("=", 1)[1]`), and probe with `urllib` from a script file — never inline shell with the token in it. (This is the same credential-shape mangling family as the GCP-redaction notes above: the in-transit redactor AND the shell both eat credential-shaped strings; the durable rule is "tokens live in root-only files, move them via Python, verify by hash".)
- **Orchestrator-down drill (the portability acceptance test):** (1) `systemctl is-active okf-mcp.service` + `curl 127.0.0.1:8910/healthz` → 200; (2) `pgrep -af "profile orchestrator"` to show the orchestrator gateway is dead/failed; (3) `kill <one of the orchestrator's stdio okf children>` → standalone service still 200 and its MCP calls still work. The already-failed orchestrator unit + live 200 healthz is itself the live proof.
- **Service indexes whatever branch the shared checkout is on (hit live 2026-08-20).** After `sudo systemctl restart okf-mcp.service`, `mcp_okf_status.head` came back as Fred's feature-branch tip (`61d7422`), not `origin/main` — the restart re-indexed the working tree, and the tree was sitting on `feature/fred-okf-hde-guest-fleet-ops`. Every MCP client then read a non-canonical tree. Rule: before restarting the service, pin the checkout (`git switch main && git reset --hard origin/main` — first confirm the current branch's work is pushed: local tip == origin tip + clean tree, so the switch loses nothing), THEN restart, THEN verify `mcp_okf_status.head == origin/main`. The service has no branch awareness; the checkout IS its source.
- **Flagged 2026-08-20:** the orchestrator gateway was running OUTSIDE systemd (unit `failed`, live process orphaned) — nothing respawns it. Opposite failure mode from OKF, but the same class of "who parents this process" audit.

## Skill hub (`okf/skills/` — added 2026-08-20, PR #33)

`okf/skills/` is the **git-versioned registry of every skill** (Hermes 12 profiles + AGY CLI `~/.antigravity/skills/` + 4 engine stores). 283 SKILL.md / 176 unique names; 12 divergent (reconciliation backlog in `okf/skills/index.md` ⚠ section). Standard: `okf/standards/okf-skill-hub.md`; decision: `okf/decisions/2026-08-20-okf-skill-hub-phase-a.md`. Regenerate after skill changes: `python3 scripts/skill-hub-snapshot.py` (idempotent, marker-guarded; `SKILL_HUB_ROOT` env var overrides the hub path for portability/sandboxing) — the index diff IS the change report. Never hand-edit generated files. **Post-merge state (2026-08-20):** PR #33 MERGED (squash `64ed7bc`); daily no-agent cron **`okf-skill-hub-regen`** 07:00 UTC runs `profiles/kai/scripts/skill-hub-regen.sh` (silent-when-clean; commits+pushes only on real changes; respects the push guard — holds the commit local and reports if blocked). PR #34 fixed the churn bugs: `generated_at` is now derived from the last commit date touching real skill files (no-op runs are **byte-stable**), `.usage.json` runtime telemetry is excluded, marker = one guard per source root (hermes/ wipe removes all profile subdirs). Phase B = [GRO-4817](https://linear.app/growthwebdev/issue/GRO-4817): engine `prismatic skills sync --source <okf-checkout>` + hermes/agy backends — the engine reads a plain git checkout, never the MCP. **Phase A loop closed 2026-08-20:** PR #34 merged (squash `6bdc289`), MCP live at that head (2,663 docs / skills=2,240), daily cron live, GRO-4817 filed. **Drift reconciliation DONE (2026-08-20, PR #35):** 72 multi-copy names, only 12 truly divergent (60 byte-identical copies — no action). Method: census via `rglob("SKILL.md")` + sha256-per-name → per-name variant table (count, hash, profile list) → per-minority-variant `diff --unified=0` line counts + sample hunk. Canonical selection rules: majority wins when it's clearly the superset; **never unify host-specific facts** (tailscale IPs/ports, per-profile llama.cpp ports) — different data, not drift; **never unify different distributions** (AGY live store vs prismatic portable store — different artifacts by design); unify only true copy-drift. Execution: backup each target dir (`/tmp/skill-reconcile-backup-<date>/<profile>/<skill>/orig`), mirror canonical dir (wipe non-marker files, copytree), regen hub, verify `divergent` count dropped (12→7), PR with decision doc in `okf/decisions/`. Left divergent by design: `tailscale-lan-access`, `qwen-llamacpp-reasoning-effort` (host-specific), `agy-oauth-authentication`/`autonomous-execution-discipline`/`golden-thread` (store distributions), `daily-transit-briefing`/`hermes-agent` (genuinely debatable — owner review). Full session record + table in `references/skill-drift-reconciliation-2026-08-20.md`. **Post-merge regen drift is EXPECTED, not a bug:** a concurrent agent editing skills between snapshot and merge produces a genuine diff when you regen on main (hit live: 17 files — new reference docs, 1 rename, content edits). Leave the tree clean; the daily cron auto-commits+pushes it, or open a PR if it should land sooner. Session records: `references/skill-hub-phasea-2026-08-20.md` + `references/skill-hub-regen-cron-and-sandbox-verification.md`.
...
- **Per-profile staleness** — fixing the index for one profile does not fix the others; each runs its own process. **Refresh recipe (hit live 2026-08-23, PR #43 post-merge):** the per-profile OKF MCP is usually a *stdio* child of that profile's gateway (born at session start, in-memory `rglob` index at boot — no mtime checks). After a merge lands on disk, `mcp_okf_read` works (reads disk) but `mcp_okf_list`/`mcp_okf_search` miss the new doc, and restarting the standalone `okf-mcp.service` (HTTP :8910) changes NOTHING because your session isn't talking to it. Diagnose: `ps aux | grep okf-mcp-server/server.py`, find the pid whose ppid is YOUR profile's gateway (`-m hermes_cli.main --profile kai gateway run`), and compare its boot time vs `git log -1` on the checkout. Fix: `kill <that pid>` — Hermes respawns it on the next MCP call with a fresh index; verify by `mcp_okf_search` on a unique term from the new doc (hit `mtime` in the result should equal the file's actual mtime, not the process boot date).
...
- **Branching off a stale local main drags unpushed commits into the PR (hit 2026-08-23, PR #43)** — local `main` can be *ahead* of `origin/main` (e.g. an unpushed `okf/skills` auto-regen) AND behind it at once. Branching from local `main` then bakes those foreign commits into the PR base (I saw 50+ `okf/skills/**` files appear in a 2-file PR). Fix: branch from `origin/main`, not local `main`; before pushing verify `git rev-list --count origin/main..HEAD` is exactly the number of commits you intended; after a force-push, GitHub's `files` field can lag ~10s — re-query `gh pr view` before concluding the PR is wrong.
...
Reference: `references/linear-api-patterns.md` — Linear GraphQL working mutation shapes (issueCreate priority=Int, issueUpdate id+input split, GRO state UUIDs, app-key can't assign to PE bot user) + bulk-creation pattern. `references/aot-hub-centralization-phase1-2026-08-19.md` — session record for the first consolidation (AOT → `okf/hubs/active-oahu/`), source inventory, secret-scan incident, open follow-ups. `references/okf-phase3-spoke-migration-2026-08-19.md` — Phase 3 full-repo census table, migration transform, env quirks, open items. `references/okf-phase4-adoption-plan-2026-08-20.md` — Phase 4 adoption plan + verified recon state (redundant-branch census, deferred-gap branch names, darius-star pointer/legacy gotcha, open decisions). `references/skill-hub-phasea-2026-08-20.md` — skill-hub Phase A: okf/skills/ registry design, generator, drift status, Phase B/C plan, pitfalls. `references/skill-hub-regen-cron-and-sandbox-verification.md` — daily regen cron contract, PR #34 churn fixes (stable generated_at, .usage.json exclusion, marker scheme), and the sandboxed repo-writing-script verification recipe with its pitfalls. `references/skill-drift-reconciliation-2026-08-20.md` — 12-way drift reconciliation: census method, disposition rules (host-specific vs distribution vs true drift), execution + verification. `references/okf-standalone-service-portability-2026-08-20.md` — okf-mcp.service facts, HTTP-client wiring schema, the token-handling 401 diagnosis, orchestrator-down drill, and the branch-pointer race recovery. `references/cross-lane-code-handoff-packet-2026-08-21.md` — the 5-step "commit locally + hand off in-lane" pattern when the lane guard blocks code in another agent's lane (embedded byte-verified patch in an in-lane OKF doc + PR + Linear child task; verification recipe).
