...
1. **Server side (model host).** llama.cpp: one key per line in a `0600` file + `--api-key-file /path/.api_keys` in the launcher (key is read at startup — restart after changing it). vLLM: `API_KEY=*** — on vLLM 0.27.1 use `--api-key "$(cat /path/.api_keys)"` in the launcher, which is a **single value, last-wins** (`--api-key A --api-key B` keeps only `B`; `--api-key A,B` keeps the literal `A,B` — no comma split, no multi-key flag). `GUARDED_PREFIX=(/v1,/v2,/inference,/cohere)`: once `--api-key` is set, `/v1/*` requires it but `/health` and root stay open; a keyless vLLM is permissive (200 on any `Bearer`). Note: on the VM232 llama.cpp build `/v1/models` is NOT gated but `/v1/chat/completions` IS — a 200 on `/v1/models` without a key does not mean auth is off. Test enforcement with a 1-token completion: correct key → 200, no key → 401, wrong key → 401.
2. **Client side (Hermes profile).** Generate `openssl rand -hex 24` with an agent prefix (e.g. `kai-llama-…`); put it in `~/.hermes/profiles/<profile>/.env` as `<PREFIX>_API_KEY=*** and switch the provider block from literal `api_key:` to `api_key_env: <PREFIX>_API_KEY`. **Restart requirement is path-dependent (verified 2026-09-06):**
   - **Main-chat path (`api_key_env`) needs NO gateway restart.** The main client resolves the key via the per-turn secret scope — `gateway/run.py:2241` installs `build_profile_secret_scope` → `load_env_file(<home>/.env)`, which re-reads the profile `.env` **from disk every turn**. A 9-day-old gateway whose `/proc/<pid>/environ` has *zero* `VLLM_*` vars still resolves the real key and authenticates against a freshly-keyed server — only the model server (remote vLLM) restarts. So for a `:8000`/`:8003`-backed main model, swap the `.env` value, `daemon-reload` (harmless), and key the server: the running gateway picks the new key up on the next turn. (This refines the older blanket "you MUST restart the profile gateway" claim, which is only strictly true for the aux path below.)
   - **Aux path (`key_env`) still needs a restart + `EnvironmentFile=`.** The aux/named-custom resolution (vision / compression / title / curator) reads **`key_env`** via raw `os.getenv` → the gateway's **process env**, not the per-turn scope. If the key isn't in the process env (no `EnvironmentFile=` in the unit, or a stale process from before the `.env` write), every aux call 401s. So for aux-facing keys, add `EnvironmentFile=` and restart the gateway. (The 2026-08-22 Kai 401 was exactly this aux path.)
   - **Verify which path a flipped key serves before deciding restart scope:** main-chat `api_key_env` (scope, no restart) vs aux `key_env` (env, restart). A quick scope probe proves the main path without a restart (see the recipe in `references/` / the vLLM rollout note): build the profile secret scope in a fresh interpreter and confirm `get_secret('VLLM_…_API_KEY')` returns the real key, not a stub.
...
- **Key rotation:** llama.cpp supports multiple keys in the file — append the new key, restart, swap the profile `.env`, restart the gateway, then drop the old key line + restart. **vLLM cannot rotate this way** — `--api-key` is single/last-wins (leg 1), so there is no append-and-overlap window: swap the server key and every client `.env` in one coordinated move, or use the zero-downtime ordering below.
- **Zero-401 ordering for a fleet-wide key (verified 2026-09-05 VM230 vLLM keying):** when a keyless server is served by N running gateways, turn on enforcement LAST. (1) Write the real key into every consumer `.env` AND point each provider block at it via `api_key_env`/`key_env`; (2) **restart the consumer gateways first** — they now send the new key to a still-unkeyed (permissive) server, which 200s, so nothing 401s; (3) only then key the server (`--api-key` + restart). Keying the server before every client is already on the new key is the one ordering that 401s the whole fleet. Verify each stage with `curl /v1/models`: keyless goes 200→401 as enforcement turns on, with-key stays 200 throughout.
- **Placeholder-stub trap — verify secrets by hash, never by eye.** When you script a secret write (e.g. appending `KEY=...` to a dozen `.env` files), the value can silently land as a placeholder (a 3-char `key` stub) instead of the real 50+-char key, and the display layer scrubs secret values anyway, so an eyeball `cat`/`grep` will NOT reveal the difference. Always re-read each file and compare `sha256(value)` against the sha256 of the known-good source (the key file), printing only the hash + a MATCH/MISMATCH verdict. This is what caught the all-profiles-stub bug before the server flip would have 401'd the entire fleet.
- **Keying the instance that is serving your own session is self-referential — but for the MAIN path it is simpler than it looks (verified 2026-09-06 on Ned's `:8003`).** Because the main-chat key resolves per-turn from `.env` on disk (scope, no gateway restart needed), you do **NOT** have to restart your own gateway to pick up the new key. The only move is **restart the remote vLLM** (`vllm-ned.service` on 192.168.1.230) to apply its `--api-key`; that can run **inline** from the in-gateway terminal (ubuntu, ssh works) and block until the remote is verified keyed+up before returning. The in-gateway guard is satisfied because the flip script touches only the remote `vllm-*` unit, never `hermes-gateway-*`. Proof-of-life: if the session that fired the flip is still responsive afterwards, the per-turn scope delivered the real key and the self-flip is safe. **Only if the flipped key also serves the AUX path** (compression/title on the same profile) do you additionally need the `EnvironmentFile=` + gateway restart (which *does* require the detached-unit + fragment-built unit-name dance, since you can't restart your own gateway inline).
  - **`systemd-run` detached jobs run as ROOT and root has NO ssh key to 192.168.1.230** (only ubuntu's key is authorized). A `systemd-run --unit=... /tmp/flip.sh` that sshes to the GPU node fails every connection with `Permission denied (publickey,password)`. For one-shot *remote* flips, prefer the inline in-gateway-terminal path (runs as ubuntu) over a root detached unit — the detached-unit pattern is only the right tool when you genuinely must restart your OWN gateway (aux case) and can't do it inline.
- **Current keyed deployments:** see the OKF doc `okf/standards/local-llm-api-key.md` — it is the live source of truth; don't re-enumerate the VM/port list here, it drifts with every swap.
