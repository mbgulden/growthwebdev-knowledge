...
- **`du -sh` size lies for hollow scaffolds — count real files before reporting what's in a dir or what deleting loses (verified 2026-09-05).** A 17M `/tmp/<project>` dir can be 100% empty directory skeleton + a `node_modules` with **zero** regular files (its 63 "files" being only npm `.bin/` symlinks), and a 12K "verification" dir can be one **dangling symlink** + empty `__pycache__`. Audit recipe: `find <dir> -type f -not -path '*/node_modules/*' -not -path '*/__pycache__/*' | wc -l` (real content), `find <dir> -type f -path '*/node_modules/*' | wc -l` (package residue; 0 = hollow), `find <dir> -type l` + `readlink` + `test -e` (symlinks, dangling check), `du -sb` per top-level entry (true bytes, not `du -sh` display). Answer "what's in there / what do we lose" from the file counts, not the size — "17M, keep?" is the wrong question. A hollow scaffold from a merged PR batch is safe to delete with zero loss. This is the /tmp twin of the skill's core rule: encode "safe to remove" as a mechanical check (file count), not an inference. See `references/2026-09-hollow-tmp-scaffold-audit.md`.
...
- `references/2026-09-hollow-tmp-scaffold-audit.md` — /tmp scaffold audit: `du -sh` size lies for hollow scaffolds (17M of node_modules with zero regular files; "files" that are npm `.bin` symlinks; dangling-symlink verification dirs). The file-count recipe that answers "what's in there / what do we lose" before deletion.
