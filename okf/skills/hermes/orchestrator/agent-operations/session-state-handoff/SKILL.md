...
**Critical finding (2026-07-27, Hermes 0.17.0) — RESOLVED/INVERTED 2026-09-07:** Pattern A's `prefill_messages_file` IS loaded by the runtime (verified live 2026-09-07: the prior session's handoff greeting reached the model at cold start, proving the inject path fires end-to-end). The earlier "declared but never loaded / dormant" conclusion was **wrong** — it confused a *script* bug for a *runtime* bug. The real 2026-09-06 defect: `wire_cold_start.py` called `subprocess.run` without `import subprocess`, so every rewire silently `NameError`-d and prefill froze (11 days stale on ned). Fixed by adding the import to the canonical (orchestrator, symlinked-by-all) copy. Standing rule: always write handoffs via `write_and_wire.py` (or run `wire_cold_start.py wire` after a bare `handoff.py write`) so `current.json` and the prefill never drift. Probe with a unique marker phrase before declaring any mechanism wired/dormant.
...
- **Nested `$HOME` shells break `~`-based handoff CLI paths (observed 2026-08-26, orchestrator profile).** On hosts where the agent shell's `$HOME` is nested (e.g. `/home/ubuntu/.hermes/profiles/orchestrator/home`), `HAND=~/.hermes/.../handoff.py` resolves to a double-nested, nonexistent path (`.../orchestrator/home/.hermes/...`) and the write fails with "can't open file". Use the absolute host path instead: `python3 /home/ubuntu/.hermes/profiles/orchestrator/skills/agent-operations/session-state-handoff/scripts/handoff.py ...`. The CLI itself derives the correct canonical `state/` root from `__file__`, so once you invoke it with the absolute path the write lands correctly — only the shell-side `~` expansion is the bug. Tell-tale of a past bad write: `previous_handoff` containing a doubled `profiles/<p>/home/.hermes/...` segment.
...
- **Prefill entries MUST use `role: user` — never `role: system` (vLLM 400 class, fixed 2026-08-26).** vLLM accepts exactly one system message, at position 0. Hermes injects prefill *after* the real system prompt (conversation_loop.py ~842), so a system-role prefill produces a dual-system payload → HTTP 400 "System message must be at the beginning" on every turn → Hermes' fallback chain silently routes the whole profile to the configured cloud model (local-endpoint profile running on Gemini; token burn is the tell). `wire_cold_start.py` `_build_prefill_messages()` was fixed (2026-08-26) to emit user role; full diagnosis + fix recipe: `profiles/kai/skills/devops/hermes-profile-ops/references/vllm-prefill-system-message-400.md`. If a profile's prefill shows system role (hand-written or pre-fix), rewrite to user role, re-probe the endpoint with the post-fix shape, then restart that profile's gateway (prefill loads once at GatewayRunner init — file edit alone has no effect on the running process). **The same leg applies to a *healthy* rewire (2026-09-07):** after the `wire_cold_start.py` import fix + all-profile rewire at 02:59 UTC, the still-running ned gateway (up since 08-27) kept injecting the 11-day-old pre-freeze greeting — the running process reads the prefill file once at GatewayRunner init, so "wired on disk" ≠ "what the model sees" until the next gateway start. Detect: compare the gateway process start time (`ps -o lstart -p <pid>` for `hermes … gateway run`, or the `start_time` epoch in `state/gateway.heartbeat`) against the prefill mtime — process older than file ⇒ the new prefill only lands after a restart. Check the whole fleet, not just the profile you fixed (all 6 gateways predated the rewire). A gateway restart is a lifecycle change ⇒ ask Michael first; roll one at a time.
...
1. **Detect.** Compute `now - written_at_utc`. If >4h, treat the handoff as **stale-not-trustworthy** and label it explicitly in the first reply: "current.json was last written at <utc>, which is N hours before this session." **Also check the producer** (`agent` / `written_by` / `session_id`): a stale handoff is often written by a no-agent cron job from another agent line (observed 2026-08-26: a 5-day-stale handoff was from the Becca journal cron — `agent: becca-journal-recap`, `written_by: cron_no_agent`). That handoff describes *that pipeline's* state, not the human's work; its `next_action` is the job's next tick, not Michael's next thing. Greet with the stale label + what you actually found on disk, and let the user's new message drive the turn — never execute the stale next_action.
