...
**If the env comes from a wrapper script, export the default with the NO-COLON form** (verified live 2026-08-26): `export GDRIVE_SA_KEY="${GDRIVE_SA_KEY-/path/key.json}"` — the `${VAR-default}` (no colon) substitutes only when the variable is UNSET, so an explicit `GDRIVE_SA_KEY=` (empty) from the environment still selects the OAuth fallback. A hard `export GDRIVE_SA_KEY=/path/key.json` or `${VAR:-default}` (colon) substitutes even when the variable is set-but-empty, silently destroying the documented empty-env rollback path. This exact failure was caught by the dual-direction e2e run (hard export → OAuth-fallback e2e FAIL, identity still the SA; no-colon fix → ALL PASS in both directions). Probe to confirm semantics: `GDRIVE_SA_KEY= ./node_wrapper.sh -e "const v=process.env.GDRIVE_SA_KEY; process.stdout.write('RAW:['+(v===undefined?'UNDEF':v)+']'+(v!==undefined && v===''?' IS_EMPTY':''))"` → expect `RAW:[] IS_EMPTY`.
...
- **A token issued with a LIMITED scope cannot be broadened by refresh — it 400s `invalid_scope`, and every WRITE the app tries will 403 (or silently no-op).** Symptom this session: gdrive token was `drive.file` (read-only-ish / app-created-files-only); moving files returned 200 but did nothing, and a refresh-grant attempt to add `auth/drive` returned `400 invalid_scope`. There is no silent "upgrade the scope" — the ONLY path is a full re-consent with the larger scope (the one click), then exchange the new code for a token carrying the full scope. Do not burn turns trying to "refresh into" a bigger scope.
- **When you write a token file after a code exchange, preserve the EXISTING `refresh_token` and keep the canonical shape — or you will manufacture the very "re-auth over and over" loop you were trying to end.** (Observed 2026-09-07, gdrive.) Google's refresh grant returns an EMPTY `refresh_token` (it's only issued at consent), so `tokens.refresh_token || old.refresh_token` — never trust the new one to be present. And `server.js`-style loaders expect `scope` as a **space-separated STRING** plus `expiry_date` (ms epoch); writing `scope` as a JSON array (the natural shape from `getToken`) trips the loader and forces re-auth. Canonical saved shape: `{ access_token, refresh_token, scope: "<space-joined string>", token_type, expiry_date: <ms>, refresh_token_expires_in, id_token }`. Verify the saved file's `scope` is a string and `refresh_token` is non-empty before declaring re-auth done.
- The MCP server may need a restart if it caches the OAuth client object. Most local-subprocess MCPs re-read the token at request time, but a few cache OAuth2 instances at startup. If re-probing still fails after the token file is rewritten, the user restarts Hermes (outbound action gate). **Cheaper in-session path (verified 2026-08-26): Hermes auto-respawns a killed MCP stdio child.** To change a server's ENV (e.g., flipping the auth-mode env var after patching the wrapper), just `kill -TERM <node-child-pid>` (find it: `ps -eo pid,ppid,args | grep 'mcp/server.js'`, the child of the profile's gateway PID) and make any MCP tool call — the reconnect-with-backoff path respawns the child with the new env within ~30s. No gateway restart, no outbound action. Verify with `/proc/<new-pid>/environ` (grep the var) + a live tool call + a direct API probe (different identity/zero-visibility = mode actually flipped).
...
- **A pasted redirect missing the `http://` prefix silently breaks `extractCode()` (observed 2026-08-26, gdrive).** Users pasting from mobile/Telegram often drop the scheme: `localhost/?iss=...&code=4/...`. Since it doesn't start with `http://`, `extractCode()` treats the ENTIRE string as the bare code → Google returns `invalid_grant` and the token file is untouched. Fix: pass just the bare `4/...` code. In the observed case the code survived the failed attempt, so an immediate bare-code retry succeeded — but codes are single-use/short-lived, so retry fast and, if the second attempt also fails, treat it as consumed and request fresh re-consent.
