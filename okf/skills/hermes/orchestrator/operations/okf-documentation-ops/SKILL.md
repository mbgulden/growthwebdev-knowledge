...
- Do **not** commit to whatever branch the hub checkout happens to have checked out. The growthwebdev-knowledge working tree can sit on someone else's unpushed local branch (2026-08-20: `content/kai-okf-phase3-spokes`, ahead 2 of origin/main, no remote) — a plain `git commit && git push origin main` there lands nothing and muddies a peer's WIP. Pattern: `git fetch` → `git checkout -b feature/<agent>-<slug> origin/main` → cherry-pick/apply only the intended docs → push the feature branch. Hub main is manual-merge by governance; feature branch + push is the complete landing. Never force-push or move another agent's local branch. **The hub also has a concurrent background writer** (an OKF skill-hub auto-regen / drift-reconciliation process that commits `okf/skills: auto-regen …` and creates `content/*` + `rescue/*` branches on its own). The remote branch will often have moved between your `fetch` and `push`, so a non-fast-forward rejection is *expected* — not an error. Do not merge blindly and do not bare-`--force`: rebuild from a **fresh** `origin/main`, prove your tip is a verified **superset** of the remote tip, then `--force-with-lease` on **your own** `feature/<agent>-*` branch only, and read back from `origin/` (the working tree is unreliable mid-session). The repo's `git config user.name` may be another agent's (it was "Ned") — the lane hook keys off the **branch prefix**, not the author, so `feature/*` → fred → `owner: ["*"]`. **Lane = branch prefix, full stop (2026-09-13 hard failure):** `PRISMATIC_ENGINE.yaml` maps *every* branch prefix to an agent (fred `feature/`, kai `content/`, agy `research/`, jules `jules/`, ned `ned/`, george `george/`) and the pre-push hook resolves the agent from the **current branch's prefix** — not the commit author, not the branch's human-readable name. Pushing from a `content/fred-…` branch therefore validates as **Kai** (lanes: `okf/hubs/`, `okf/standards/`, `okf/audits/`, …) and rejects `okf/reports/` + `okf/integrations/` with `Lane violation by kai`. Symptom to recognize: a push of 100% of YOUR changes, on a branch you created, fails on files inside a directory you clearly own. **Diagnosis recipe:** `python3 -c "import yaml; c=yaml.safe_load(open('PRISMATIC_ENGINE.yaml')); [print(a, v.get('branch_prefix'), v.get('lanes',{}).get('owner')) for a,v in c['agents'].items()]"` in the repo root — if your branch prefix isn't yours, that's the whole story. **Fix:** create `feature/<agent>-<slug>` from the same tip, push the PR from there; the old `content/` branch stays as the polluted PR (close/supersede at merge time). Same rule applies to the PE repo (`prismatic-engine`): doc/CLI work goes on `feature/fred-*`, never `content/*`. **Rebase-conflict recovery (2026-09-12):** when `git rebase` hits a conflict in an *unrelated* file (e.g. a Ned skill auto-regen file), do NOT try to resolve it. `git rebase --abort`, then `git checkout -b <your-branch> origin/main`, then `git checkout <your-merge-commit> -- <your-file>` to extract just your file, commit, push. This is the cleanest path when the conflict is in someone else's auto-regen territory. The pre-push hook also blocks pushes to `main` directly (production deployments are manual-only) — always push to a **`feature/<agent>-<slug>`** branch and open a PR (`content/*` belongs to Kai's lane, see the lane rule above). **Concurrent-landing conflict (re-validated 2026-09-13, 2nd consecutive hit):** when a peer's earlier PR (e.g. a `content/*`-branch PR) lands on `origin/main` *before* your `feature/*` PR, GitHub flags your PR CONFLICTING on **add/add** for every file both added (both branches independently added the same doc from a common ancestor). Fix recipe: on your feature branch, `git stash -u` (sweep untracked) → `git merge origin/main` → for each add/add conflict pick the **superset** version (the one with the later, more complete state — verify with `git show :2:<path>` (ours) vs `:3:<path>` (theirs) + `diff`) → `git add` → commit the merge → push. Post-push, re-check `gh pr view N --json mergeable,mergeStateStatus` and expect `MERGEABLE`/`CLEAN`. **Stale-PR-head trap (2026-09-13):** a PR's *head* can be behind the remote branch it names — `gh pr view N --json headRefOid` vs `git ls-remote origin refs/heads/<branch>` will disagree (the branch was force-pushed or advanced past the PR head). Don't trust the PR head as the branch tip; read `ls-remote`. See `references/shared-hub-concurrent-writer-landing-2026-08-20.md`.
