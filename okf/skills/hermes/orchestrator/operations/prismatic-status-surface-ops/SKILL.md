...
- **Workspace-tree preview can 400 `invalid workspace identifier` on VALID ids (strict-registry flake) while `/api/workspaces` still lists them** — that is a `WorkspaceRegistry.resolve()` registry bug, not a path/file problem. Isolate by comparing list vs resolve; never conclude the reviewed file is wrong from it. A hosted review link must be proven end-to-end (public domain, 200, sha256 of returned content == disk) before it's handed off; when it can't be, the fallback is tarball + SHA256 + Linear comment — the web link is never the only delivery path. See `references/workspace-tree-review-link-400-and-gateway-sot-2026-08-20.md`.
