...
**⚠️ CORE PRINCIPLE: Don't Trust. Verify.** The registry is a snapshot, not ground truth. Always verify project state against live Linear, git, and disk before reporting.
...
Read `$PRISMATIC_HOME/work/project-registry.json`. This is the single source of truth.
...
- **AGY watchdog**: Run `python3 $PRISMATIC_HOME/work/agentic-swarm-ops/ops/agy_watchdog.py` to check for stuck AGY processes
...
**GRO-1272 pattern (Jun 2026):** An issue stuck In Progress with agent:fred label despite all deliverables being on disk. The Lyria music tracks (40 MP3 files) existed but the Linear issue was never moved to Done. Always check disk for deliverables before creating new issues — the work may already be complete. See also Step 3.5 (Prior-Session Completion Verification).
...
- **Hermes sandbox path isolation (CRITICAL):** Hermes profiles use a sandboxed home directory (`~/.hermes/profiles/<name>/home/`), distinct from the real filesystem at `$PRISMATIC_HOME/`. The `~/work/` inside the sandbox is a SEPARATE directory — not the real `$PRISMATIC_HOME/work/`. This means `project-registry.json`, project repos, and other files may appear missing when they actually exist. **This affects BOTH `terminal()` with `~` paths AND `execute_code` sandbox code** — `os.path.expanduser("~")` in `execute_code` resolves to the sandbox home, not `$PRISMATIC_HOME`. Fix: symlink `~/work` → `$PRISMATIC_HOME/work` (one-time: `ln -s $PRISMATIC_HOME/work ~/work`). Verify with `readlink -f ~/work`. If files still appear missing after symlink, use absolute paths: `ls $PRISMATIC_HOME/work/` directly. Terminal commands using `~/work` go to the sandbox path — always verify with an absolute path if something is missing.
...
- **Execution agents picking `agent:fred` issues — filter by agent type (Jun 2026):** When an execution agent (Ned, Jules, Codex) has no agent-specific issues and falls back to `agent:fred`, it MUST skip AGY research/strategy tasks. These are not coding/execution tasks — picking one wastes a cycle on verification-only closure (the deliverables were already produced by AGY). **Detection — title prefixes to skip:** `AGY —`, `AGY:`, `AGY REVIEW`, and titles containing `[AUDIT] AGY` or `[AUDIO AUDIT] AGY`. **Also skip:** issues where the description references AGY tools (Ubersuggest, GA4, Search Console, Imagen 3, Google Flow Beta, visual QA passes). **Fix:** filter `agent:fred` results client-side by title AND description content; pick the oldest non-AGY issue. If all `agent:fred` issues are AGY tasks, silently exit. **Real example:** Ned's cron run (Jun 11) found 4 `agent:fred` issues — all 4 were AGY tasks. GRO-1178 was already complete (8 files on disk from AGY's prior run). The cycle was a verification-then-close, not execution. **Second example (Jun 12):** Ned found 18 `agent:fred` fallback issues; 11 were AGY tasks using prefixes `AGY:`, `AGY REVIEW`, or `[AUDIO AUDIT] AGY` — the original "AGY —" prefix filter missed them all.
...
- **Python `gql()` helper does NOT support GraphQL variables (NEW Jun 2026):** The stdlib `gql()` function shown in the Quick Issue Creation section sends `{'query': query_string}` — it has NO `variables` key in the JSON payload. Mutations that declare variables (`$body: String!`, `$id: String!`) will return HTTP 400 because Linear expects a separate `variables` object. **Workaround:** Use `curl` directly with inline mutation strings for `commentCreate`, `issueArchive`, and `issueUpdate` with `stateId`. Save the `gql()` helper for queries only — NEVER for mutations that use GraphQL variables. **This affected 15 consecutive comment posts in a Jun 13 orchestrator session.** See `references/agy-dispatch-batch-routing.md` for the working shell-level curl pattern.

- **Batch AGY dispatch routing (NEW Jun 2026):** When 10+ AGY dispatch result files accumulate in `/tmp/`, use the 4-phase pattern: Surface Scan → Batch Identify → Route (shell curl) → Archive. See `references/agy-dispatch-batch-routing.md` for the full playbook. 26 dispatch results routed in one 15-min cron session.
...
- **Chart tests need `timezonefinder` + correct invocation**: Running `pytest` on the OpenHumanDesignMCP test suite requires `timezonefinder` and the correct working directory. The full invocation:
  ```
  cd $PRISMATIC_HOME/work/OpenHumanDesignMCP/hd-mcp-server && \
...
### Review References
- `references/spec-compliance-review.md` — Second Witness pattern: cross-reference issues against a formal architecture blueprint, rate APPROVED/NEEDS_CHANGES/BLOCKED, create fix tasks
- `references/initiative-health-pulse.md` — Compact initiative-level dashboard format
- `references/linear-api-queries.md` — Working GraphQL queries, auth pattern
- `references/linear-initiatives-restructuring.md` — Full account reorganization pattern
- `references/duplicate-issue-patterns.md` — Known duplicates, stale templates
- `references/thread-health-scoring.md` — Scoring heuristics for 🟢🟡🔴 thread status
- `references/domain-portfolio-strategy.md` — Three-domain clean architecture
- **`prismatic-validation-pipeline` skill** — Provider-agnostic multi-agent quality loop (Worker→AGY→Fred). Fred orchestrates, agents validate each other.
- **`prismatic-engine-operations` skill** — File locking, lane gating, staging governance
