...
| **Local LLM server API key pattern** (2026-08-21, current) | [`./local-llm-api-key.md`](./local-llm-api-key.md) | n/a | Reusable auth pattern for local OpenAI-compatible model servers (VM232 llama.cpp, VM230 vLLM): server key file + profile `api_key_env` + Autobot watchdog. |
| **Local LLM GPU allocation & model topology** (2026-09-13, current) | [`../integrations/local-llm-gpu-topology.md`](../integrations/local-llm-gpu-topology.md) | GRO-4929 | Single table of record for the 4-lane local LLM fleet: who owns which GPUs (.230 Fred :8000 + Ned :8003, .232 Kai/George :8080), runtime/quant, VRAM headroom, profile→lane routing. Cross-links `local-llm-api-key.md`. |
